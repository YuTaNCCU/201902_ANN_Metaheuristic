...
class ES:
    ...
    def __init__(self, KerasModels, weights1, weights2, InitialSigma = 0.1, ParentsSize = 15, ChildSize = 100, tao = 0.5, max_steps = 100):
        """
        :param KerasModels: a Keras model, like keras.engine.sequential.Sequential
        :param weights1, weights1: initial weights, should be a Keras model weight
        :param max_steps: maximum number of steps to run algorithm for
        """
        self.KerasModels = KerasModels
        
        if isinstance(weights1, list) & isinstance(weights2, list):
            self.WeightsStrucure, self.weights1 = self._FlattenWeights(weights1)
            _, self.weights2 = self._FlattenWeights(weights1)      
        else:
        ...
        if all(isinstance(x, int) for x in [ParentsSize, ChildSize, max_steps]) and all(x > 0 for x in [ParentsSize, ChildSize, max_steps]):
    ...
    def run(self, verbose=10):
        ...
        Population_Parents_Sigma = np.full(shape = (self.ParentsSize, len(self.weights1)), fill_value = self.InitialSigma ) 
        ...
        while True:
            ...
            BestNIndex = np.array( Population_Child_score ).argsort()[::-1][:self.ParentsSize]
...
MyES = ES(model, weights1, weights2, InitialSigma = 0.1, ParentsSize = 15, ChildSize = 100, tao = 0.5, max_steps = 100)         

ES_Optimized_Model, ES_Optimized_Weights, ES_Optimized_ObjVal  = MyES.run(verbose = 10)
